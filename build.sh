...
FRANKENPHP_TAG_83="${FRANKENPHP_TAG_83:-}"
FRANKENPHP_TAG_84="${FRANKENPHP_TAG_84:-}"
FRANKENPHP_TAG_85="${FRANKENPHP_TAG_85:-}"
...
for version in ${PHP_VERSIONS}; do
  ...
  case "${version}" in
    8.3)
      ...
      frankenphp_tag="${FRANKENPHP_TAG_83:-1.3-php8.3-bookworm}"
      ;;
    8.4)
      dockerfile="Dockerfile.php84"
      frankenphp_tag="${FRANKENPHP_TAG_84:-1.3-php8.4-bookworm}"
      ;;
    8.5)
      dockerfile="Dockerfile.php85"
      frankenphp_tag="${FRANKENPHP_TAG_85:-php8.5-bookworm}"
      ;;
    *)
  ...
  echo -e "\n${GREEN}=== 构建 ${YELLOW}${tag}${GREEN} (dockerfile: ${YELLOW}${dockerfile}${GREEN}, base: ${YELLOW}${frankenphp_tag}${GREEN}) ===${NC}"
  docker buildx build \
    "${build_args[@]}" \
    -t "${tag}" \
    "${EXTRA_ARGS[@]}" \
